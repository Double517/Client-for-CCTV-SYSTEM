...
export default class CCTVView extends Component {
	...
	CCTVViewer() {
		return(
			<View>
				<Example videoURL={this.state.videoURL}/>
			</View>
	...
	emitControlMessage(action) {
		let url = "http://"+this.state.carURL;
		let header = this.state.carHeader;
		ToastAndroid.show(JSON.stringify(header), ToastAndroid.SHORT);
		fetch(url, header)
